...
public class get_probe_zero_variants {
    @UserFunction
    @Description("Variants in a specific probe.")

    public String compute_probe_variants_from_seqs(
        @Name("pid")
            Long pid,
        @Name("pos")
            Long pos,
...
//        calling function must process results based on its requirements
        String r = get_variants(pid, pos, ref_seq, test_seq);
         return r;
            }
...
//        r = get_variants(1014066L);  //1014068L);
//         System.out.println(r);
    }
    
     public static String get_variants(Long pid, Long pos, String ref_seq, String test_seq) 
    {
        ...
        DiffMatchPatch dmp = new DiffMatchPatch();
         ...
         try
         {
        diffs = dmp.diffMain(ref_seq.replace("\"",""), test_seq.replace("\"",""));
         curr_pos = pos;
...
//            r = r + gvars.get(i).name + "|" + mt_var.probes.pos_zero_pos_probes.pid_index + "|" + gvars.get(i).pos + "|" + gvars.get(i).anc + "|" + gvars.get(i).der + "|" + gvars.get(i).classification + "|" + gvars.get(i).method  + "|" + ref_seq + "|" + test_seq + "|" + sDiffs + "\n";;
//        mt_var.probes.pos_zero_pos_probes.pid_index = mt_var.probes.pos_zero_pos_probes.pid_index + 1;
        }
            return r;      
    }
     ...
     public static Long fix_pos(Long pos)
     {   //used for pos=0 
         if(pos>16569)
         {
             return pos-16569L;
