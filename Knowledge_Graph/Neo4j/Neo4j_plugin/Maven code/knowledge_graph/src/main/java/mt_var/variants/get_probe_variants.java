                     ...
                     v.pos = curr_pos;
                      ...
                      if (diffs.get(i+1).operation.compareTo(DiffMatchPatch.Operation.EQUAL)==0)
                       { // simple deletion where there is no INSERT following the DELETE
                           ...
                           //2nd: with individual deletions for each position - method 7 below
                           if(v.anc.length()>1)
                           {
...
//                                   ct = ct + 1;
//                               }
                           //3rd: look for specific patterns for deletions
//                           String sd = specific_deletion(diffs.get(i).text, v.pos, pid);
//                           if (sd.compareTo("")!=0)
//                           {
//                                   mt_var.variants.gfgVar nv = new mt_var.variants.gfgVar();
//                                   nv.pos = curr_pos;
...
//creeate arraylist to hold one iteration which is used to find boundaries of a deletion
     ...
     private static String specific_deletion(String del, Long pos, Long pid, ArrayList<mt_var.variants.gfgVar> ngg )
     {
         
         if ("[|GCTGG|, |GGAG|, |CGGAGC|, |GGAGCA|, |GCAC|, |ATTT|, |AAAC|, |CCCT|, |TTTCC|, |ACTACTAATCT|, |ACACNN|, |ACNN|, |NNCG|, |TATCTACN|, |TATAGCACCC|, |GCACCCCCT|, |CACCCCCTC|, |ACCCCCTC|, |CCCC|, |TCTA|, |CTACC|, |ACCCCCTCT|, |CCCCCTC|, |CCCCCTCT|, |CCCCCTCTAG|, |CCCCTCT|, |CCCCTCTAG|, |ACCAT|, |AAAA|, |AACCCT|, |ACCCT|, |CCCA|, |TACC|, |CACATAA|, |TGAAATCA|, |AAAGTG|]".contains("|" +del + "|") )
         {
             System.out.println(pid + "\t" + del);
         }
         switch (del)
         {
             case "CACCCCC":
